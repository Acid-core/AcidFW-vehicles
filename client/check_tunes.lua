...
exports['qb-target']:AddTargetModel(`s_m_y_xmech_02`, {
    options = {
        {
            event = "AcidFW:client:checkVehicleMods",
            icon = "fas fa-car",
...
RegisterNetEvent('AcidFW:client:checkVehicleMods', function()
	...
	if IsPedInAnyVehicle(ped, false) then
		if Config.isVehicleOwned then
			AcidFW.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
				if owned then
					AcidFW.Functions.Notify("Checking for Vehicle Mods...", "primary", time)
        ...
        AcidFW.Functions.Notify("Cant help you without a vehicle!", "error", 3500)
	end
end)
...
function checkVehicleMods()
	...
	local vehProps = AcidFW.Functions.GetVehicleProperties(vehicle)
	...
	if Config.checkTunesMessage == "chat" then
		...
		AcidFW.Functions.Notify("Vehicle Mod List Recieved", "success")
	elseif Config.checkTunesMessage == "phone" then 
		AcidFW.Functions.TriggerCallback('AcidFW:HasItem', function(HasItem)
			if HasItem then
			  ...
			  AcidFW.Functions.Notify("Mod List was sent to your phone but you dont have one", "error" , 5000)
			end
		end, 'phone')
	end
end
